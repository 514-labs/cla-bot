# CLA Bot Documentation

## Screenshot
![Documentation home screenshot](/docs/screenshots/docs-home.svg)

Choose the path that matches your role.

## Repo Contributor (`cla.fiveonefour.com`)
- [Signing a CLA](/docs/contributor/signing-a-cla)
- [See all CLAs I signed](/docs/contributor/signed-cla-history)
- [Download a CLA](/docs/contributor/download-a-cla)
- [Contributor FAQ](/docs/contributor/faq)

## Repo Admin (`cla.fiveonefour.com`)
- [Setup CLA Bot with my org](/docs/admin/setup-cla-bot-with-my-org)
- [Manage the installation](/docs/admin/manage-installation)
- [Admin FAQ](/docs/admin/faq)

## Operator
- [Deploy this on my infrastructure](/docs/operator/deploy-on-my-infrastructure)

## Source of Truth
- Product behavior: [`SPEC.md`](https://github.com/514-labs/cla-bot/blob/main/SPEC.md)
- Setup and environment: [`README.md`](https://github.com/514-labs/cla-bot/blob/main/README.md)
